...
public class QuarkShare extends Spider {
public static Map<String, JSONObject> shareTokenCache = new HashMap();
    ...
    public String homeContent(boolean filter) throws Exception {
        ...
        List<String> typeIds = Arrays.asList("1052e4b57d3d");
        List<String> typeNames = Arrays.asList("短剧每日更新");
        for (int i = 0; i < typeIds.size(); i++) classes.add(new Class(typeIds.get(i), typeNames.get(i)));

        List<Vod> list = new ArrayList<>();
//        list.add(new Vod("https://pan.quark.cn/s/1052e4b57d3d","测试", "pic"));
    ...
    public String categoryContent(String tid, String pg, boolean filter, HashMap<String, String> extend) throws Exception {
        ...
        if (tid.contains("*")) {
            ...
            shareData = split[0];
            fid = split[1];
            videos = this.getFilesByShareUrl(shareData, fid);
        } else {
            shareData = tid;
            fid = "0";
            videos = this.getFilesByShareUrl(shareData, fid);
            if (videos.size() == 1 ) {
                JSONObject v = (JSONObject)videos.get(0);
                if (v.getBoolean("dir")) {
                    videos = this.getFilesByShareUrl(shareData, v.getString("fid"));
                }
            }
        }
        for(int i = 0; i < videos.size(); i++) {
            ...
            Boolean dir = v.getBoolean("dir");
            if (dir) {
                list.add(new Vod(shareData + "*" + v.getString("fid"), v.getString("file_name"), null,v.getString("icon"),true));
            } else {
                list.add(new Vod(shareData + "*" + fid, v.getString("file_name"), null,v.getString("icon")));
            }
        }
        return Result.string(list);
    }

    public QuarkShare Quark() {
        if (Objects.isNull(instance)) {
            instance = new QuarkShare();
        }
        return instance;
    }

    public static QuarkShare get() {
        if (Objects.isNull(instance)) {
            instance = new QuarkShare();
        }
        return instance;
    ...
    public String detailContent(List<String> ids) throws Exception {
        ...
        JSONObject shareDataByUrl = this.getShareDataByUrl(shareData, fid);
        Vod vod = new Vod();
...
//                urls.add(String.format(Proxy.getUrl() + "?do=quark&what=%s&flag=%s&shareId=%s&fileId=%s&end=%s", "trans", jsonObject.getString("resolution").toLowerCase(), ids[0], URLEncoder.encode(ids[1]) + "*" + ids[2] + "*" + ids[3], ".mp4"));
    ...
    public JSONObject getShareDataByUrl(String shareData, String fid) throws Exception {
//        String shareData = this.getShareData(shareUrl);
        List videos = this.getFilesByShareUrl(shareData, fid);
    ...
    public List getFilesByShareUrl(String shareId, String fid) throws Exception {
        ...
        listFile(shareId, fid, "1", videos, subtitles);
        if (subtitles.size() > 0 ) {
            for (int i = 0; i < videos.size(); i++) {
        ...
        if (StringUtils.equals("quark", site)) {
            ...
            if (StringUtils.equals("trans", what)) {
                if (Objects.isNull(quarkTranscodingCache.get(ids[1]))) {
                    ...
                    JSONArray liveTranscoding = new QuarkShare().getLiveTranscoding(shareId, URLDecoder.decode(ids[0]), ids[1], ids[2]);
                    JSONArray transcoding = new JSONArray();
                    for (int i = 0; i < liveTranscoding.size(); i++) {
                        JSONObject jsonObject = liveTranscoding.getJSONObject(i);
                        if (jsonObject.getBoolean("accessable")) {
